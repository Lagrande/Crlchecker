...
- **Новое**: Расширенное уведомление «Добавлены новые CRL» с полями из TSL: CRL Serial Number, Issuer Key Identifier, Средство УЦ, Класс средства УЦ, Кому выдан, Кем выдан, Серийный номер, Действует, Отпечаток
- **Новое**: Отдельное уведомление о неуспешной загрузке CRL (все пробованные URL, УЦ/реестровый номер, CRL Serial Number, Issuer Key Identifier); управление через флаг `NOTIFY_CRL_DOWNLOAD_FAIL=true`
